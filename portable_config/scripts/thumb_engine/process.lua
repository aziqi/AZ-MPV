...
local ffmpeg_pending_time = nil
local ffmpeg_pending_fast = true
local ffmpeg_seek_time = nil
local frame_cache = {}             -- { [time_key] = raw_data_string }
local cache_order = {}
local cache_quantize = 5
		...
		function(success, result)
			if success == false or (result.status ~= 0 and result.status ~= -2) then
				...
				if not state.spawn_working then
					mp.commandv("show-text", "thumb_engine subprocess creation failed!", 5)
				end
