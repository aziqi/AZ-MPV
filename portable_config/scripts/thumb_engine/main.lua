...
Fork of https://github.com/po5/thumbfast/blob/master/thumbfast.lua
 ...
 <KEY>   script-binding thumb_engine/thumb_rerun
 <KEY>   script-binding thumb_engine/thumb_toggle
 <KEY>   script-message thumbnail_hwdec toggle
...
if options.load == false then
	mp.msg.info("Script has been disabled on initialization")
	return
end
local min_major = 0
local min_minor = 40
...
if helper.incompat_check(mpv_ver_curr, min_major, min_minor, min_patch) then
	mp.msg.warn("Current mpv version (" .. (mpv_ver_curr or "Unknown") .. ") is lower than " .. min_major .. "." .. min_minor .. "." .. min_patch .. ", thumbnail feature terminated.")
	return
end
...
mp.add_key_binding(nil, "thumb_rerun", function()
	...
	state.auto_run = true
	file_load(true)
	mp.osd_message("Thumbnail feature restarted", 2)
	mp.msg.info("Thumbnail feature restarted")
end)
mp.add_key_binding(nil, "thumb_toggle", function()
	if state.auto_run then
		...
		shutdown()
		file_load(true)
		mp.osd_message("Thumbnail feature temporarily disabled", 2)
		mp.msg.info("Thumbnail feature temporarily disabled")
	else
		state.auto_run = true
		file_load(true)
		mp.osd_message("Thumbnail feature temporarily enabled", 2)
		mp.msg.info("Thumbnail feature temporarily enabled")
	end
end)
mp.register_script_message("thumbnail_hwdec", function(hwdec_api)
	...
	mp.osd_message("Thumbnail preferred decoding API changed: " .. hwdec_api, 2)
	mp.msg.info("Thumbnail preferred decoding API changed: " .. hwdec_api)
	clear()
	shutdown()
	file_load(true)
end)
