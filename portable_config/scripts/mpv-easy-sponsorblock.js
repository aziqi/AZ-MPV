...
import { cacheAsync, createLogger, fetch, getProperty, getPropertyNumber, observePropertyNumber, registerEvent, registerScriptMessage, setProperty, showNotification, unobserveProperty, unregisterEvent, } from "@mpv-easy/tool";
...
let ServerList = [];
let CacheTtl = 0;
export function sponsorblock({ active: startActive, servers, categories, sponsorblockEventName, cacheTtl, }) {
    active = startActive;
    CacheTtl = cacheTtl;
...
function extractYoutubeId(videoPath, purl) {
    const patterns = [
...
async function fetchRanges(videoId) {
    for (const s of ServerList) {
        const url = `${s}?categories=${CategoriesString}&videoID=${videoId}`;
        try {
            const text = await cacheAsync(url, () => fetch(url, { redirect: "follow" }).then((r) => r.text()), CacheTtl);
    ...
    showNotification(`[Sponsorblock] all servers failed\nplease check if the servers are available: ${ServerList.join("\n")}\nand your network connection`, 5);
    return;
}
let IdCache;
function getId() {
