--[[
DOCS_ playlist_osd.conf

Trimmed from: https://github.com/jonniek/mpv-playlistmanager/blob/master/playlistmanager.lua
Only the most essential navigation features are retained

Example key bindings (add to input.conf):
 <KEY>   script-binding playlist_osd/display   # Display the advanced playlist

 <KEY>   script-message-to playlist_osd random   # Jump to a random playlist entry
...
if settings.load == false then
	msg.info("Script disabled at initialization")
	return
end
-- Reason: forgotten
local min_major = 0
...
if incompat_check(mpv_ver_curr, min_major, min_minor, min_patch) then
	msg.warn("Current mpv version (" .. (mpv_ver_curr or "unknown") .. ") is below " .. min_major .. "." .. min_minor .. "." .. min_patch .. ", script terminated.")
	return
end
