...
DOCS_ thumbfast.conf

New thumbnail engine compatible with multiple OSC-style scripts

Example keybindings (add to input.conf):

 <KEY>   script-binding thumbfast/thumb_rerun    # Restart thumbnail fetching (can be used to manually fix stuck thumbnails)
 <KEY>   script-binding thumbfast/thumb_toggle   # Toggle thumbnail preview on/off
 <KEY>   script-message thumb_hwdec toggle       # Toggle hardware decoding for thumbnails (replace {toggle} with a specific decoding API)
...
-- Reason: --load-osd-console was renamed to --load-console
local min_major = 0
local min_minor = 40
local min_patch = 0
...
if incompat_check(mpv_ver_curr, min_major, min_minor, min_patch) then
    mp.msg.warn("Current mpv version (" .. (mpv_ver_curr or "unknown") .. ") is below " .. min_major .. "." .. min_minor .. "." .. min_patch .. ", thumbnail functionality has been disabled.")
    return
end
...
mp.add_key_binding(nil, "thumb_rerun", function()
    ...
    auto_run = true
    file_load()
    mp.osd_message("Thumbnail function restarted", 2)
    mp.msg.info("Thumbnail function restarted")
end)
mp.add_key_binding(nil, "thumb_toggle", function()
    if auto_run then
        ...
        shutdown()
        file_load()
        mp.osd_message("Thumbnail function temporarily disabled", 2)
        mp.msg.info("Thumbnail function temporarily disabled")
    else
        auto_run = true
        file_load()
        mp.osd_message("Thumbnail function temporarily enabled", 2)
        mp.msg.info("Thumbnail function temporarily enabled")
    end
end)
mp.register_script_message("thumb_hwdec", function(hwdec_api)
    ...
    mp.osd_message("Thumbnail preferred decoding API changed to: " .. hwdec_api, 2)
    mp.msg.info("Thumbnail preferred decoding API changed to: " .. hwdec_api)
    clear()
    shutdown()
    file_load()
end)
