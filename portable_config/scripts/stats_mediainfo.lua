--[[
DOCS_ https://github.com/hooke007/mpv_PlayKit/discussions/624

Parse current file information using MediaInfo (excluding external or appended merged tracks) and output to OSD
]]
				...
				if data[i] ~= nil then
					if type(data[i]) == "table" then
						if data[i]["@type"] then
							...
							table.insert(lines, "")
							table.insert(lines, indent .. "{\\b1\\c&H00CC00&}[" .. track_type .. "]{\\c&H0080FF&\\b0}") -- track type heading
			...
			for _, k in ipairs(sorted_keys) do
				...
				if type(v) == "table" then
					if k == "media" and v.track then
						table.insert(lines, indent .. "{\\b1\\c&HFF8000&}MEDIA INFO{\\c&H0080FF&\\b0}") -- sub-heading
...
local function on_file_loaded()
	...
	if current_path and current_path ~= new_path then
		clear_cache()
		clipboard_cache = {} -- clear clipboard text cache
