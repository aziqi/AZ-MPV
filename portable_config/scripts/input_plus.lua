--[[
DOCS_ https://github.com/hooke007/mpv_PlayKit/discussions/615

Shortcut command enhancements
]]
...
function chap_skip_check(_, value)
	...
	for _, words in pairs(chap_keywords) do
		if string.match(value, words) and chap_skip then
			...
			mp.msg.info("chap_skip_check: skipping chapter")
		end
	end
end
function chap_skip_toggle()
	if chap_skip then
		...
		chap_skip = false
		mp.osd_message("[input_plus] OP/ED auto-skip disabled", 1)
		return
	end
	mp.observe_property("chapter-metadata/TITLE", "string", chap_skip_check)
	chap_skip = true
	mp.osd_message("[input_plus] OP/ED auto-skip enabled", 1)
end
...
function info_get()
	...
	local txt = (
	style_generic.."Config dir: ".."{\\fs18\\1c&H0099FF}"..conf_dir:gsub("\\", "/").."\n"..
	...
	style_generic.."Frame drops: ".."{\\1c&H0099FF}"..mp.get_property_number("frame-drop-count", 0).."\n"..
	...
	style_generic.."-   Pixel: ".."{\\1c&H03A89E}"..pix_fmt.."\n"..
	style_generic.."-   Range: ".."{\\1c&H03A89E}"..color_lv.."\n"..
	style_generic.."-   FPS: ".."{\\1c&H03A89E}"..fps_o.." FPS (original) "..fps_t.." FPS (target)".."\n"..
	...
	style_generic.."Shaders: ".."{\\fs18\\1c&HFF8821}"..mp.get_property_osd("glsl-shaders"):gsub(":\\", "/"):gsub(":/", "/"):gsub("\\", "/"):gsub(";", " "):gsub(",", " "):gsub(":", " ").."\n"..
...
function mark_aid_reset()
	...
	mp.osd_message("[input_plus] Track merge and marks cancelled", 1)
...
function mark_aid_A()
	...
	then
		...
		mp.osd_message("[input_plus] Pre-marked audio track " .. marked_aid_A .. " as parallel track A", 1)
	end
	if mark_aid_reg then
		return
	else
...
function mark_aid_B()
	...
	then
		...
		mp.osd_message("[input_plus] Pre-marked audio track " .. marked_aid_B .. " as parallel track B", 1)
	end
	if mark_aid_reg then
		return
	else
...
function mark_aid_merge()
	...
	then
		...
		mp.osd_message("[input_plus] " .. "A/B audio tracks merged", 1)
		merged_aid = true
	end
end
...
function scale_recal(pct)
	...
	if w_dp == 0 or w_vf == 0 or scale_win == 0 then
		mp.msg.warn("scale_recal: missing required conditions")
		scale_target = 0
		return
	end
...
function pip_dummy(pct)
	if mp.get_property_native("idle-active") or not mp.get_property_native("vid") then
		mp.msg.warn("pip_dummy: cannot be used in current state")
		return
	end
	scale_recal(pct)
...
function playlist_order(mode, re)
	if shuffling then
		mp.msg.info("playlist_order: rapid shuffle blocked")
		return
	end
...
function playlist_random()
	...
	if range <=2 then
		mp.msg.info("playlist_random: playlist has 2 or fewer items")
		return
	else
...
function playlist_tmp_save()
	...
	mp.msg.info("playlist_tmp_save: config folder/playlist_temp.mpl")
	file:close()
end
...
function quit_real()
	if pre_quit then
		...
		mp.osd_message("Run again to confirm quit", 1.5)
		pre_quit = true
		mp.add_timeout(1.5, function()
			pre_quit = false
			mp.msg.verbose("quit_real: accidental quit key detected")
		end)
	end
end
function quit_wait()
	if pre_quit then
		...
		pre_quit = true
		mp.osd_message("Quitting soon, run again to cancel", 3)
...
-- Alternative version: https://github.com/mpv-player/mpv/issues/11589#issuecomment-1513535980
...
function speed_auto(num, any)
	...
	local function speed_auto_sub(flag_complex)
		...
		if evt == "down" then
			...
			mp.msg.verbose("speed_auto: variable speed playback active")
		elseif evt == "up" then
			mp.set_property_number("speed", 1)
			mp.msg.verbose("speed_auto: normal speed restored")
		end
	end
...
function speed_adaptive()
	...
	if (fps_raw == 0 or fps_dp == 0 or fps_raw > 32 or math.abs(fps_vf - fps_raw) > 0.5) then
		mp.msg.warn("speed_sync_toggle: exceptional FPS condition detected")
		return
	end
	for i = 1, spd_iters_max do
		...
		if spd_target then
			if math.abs(spd_target - spd_cur) < 0.0001 then
				...
				mp.msg.info("speed_sync_toggle: setting speed to " .. spd_target)
				break
			end
		end
	end
...
function speed_sync_toggle()
	...
	if spd_adapt then
		speed_adaptive()
		mp.osd_message("[input_plus] " .. "Adaptive speed enabled", 1)
		mp.register_event("playback-restart", speed_adaptive)
	else
		mp.unregister_event(speed_adaptive)
		mp.set_property_number("speed", 1)
		mp.osd_message("[input_plus] " .. "Adaptive speed disabled", 1)
		return
	end
end
...
function track_refresh(id)
	...
	if current_id == 0 then
		mp.msg.warn("track_refresh: current " .. id .. " is invalid")
		return
	end
...
-- Alternative implementation: https://github.com/mpv-player/mpv/pull/11444#issuecomment-1469229943
...
function prop_hold(prop)
	local function prop_auto(flag_complex)
		...
		elseif evt == "up" then
			...
			mp.msg.verbose("prop_hold: restored")
		end
	end
	return prop_auto
end
