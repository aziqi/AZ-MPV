...
function Menu:render()
	...
	local function draw_menu(menu, x, pos)
		...
		-- Background
		local menu_rad = state.radius > 0 and math.min(state.radius + self.padding, state.radius * 3) or 0
		ass:rect(bg_rect.ax, bg_rect.ay, bg_rect.bx, bg_rect.by, {
			color = 'ffffff',
			opacity = menu_opacity * 0.3,
			radius = menu_rad,
		})
		ass:rect(bg_rect.ax + 1, bg_rect.ay + 1, bg_rect.bx - 1, bg_rect.by - 1, {
			color = bg,
			opacity = menu_opacity * config.opacity.menu,
			radius = math.max(0, menu_rad - 1),
		})
