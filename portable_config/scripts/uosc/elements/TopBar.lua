	...
	if options.top_bar_controls then
		...
		for _, button in ipairs(self.buttons) do
			...
			ass:rect(bg_ax, bg_ay, bg_bx, bg_by, {
				color = 'ffffff', opacity = visibility * opacity * 0.3, radius = state.radius,
			})
			ass:rect(bg_ax + 1, bg_ay + 1, bg_bx - 1, bg_by - 1, {
				color = button_bg, opacity = visibility * opacity, radius = math.max(0, state.radius - 1),
			})
	...
	if main_title or state.has_playlist then
		...
		if state.has_playlist then
			...
			if opacity > 0 then
				...
				ass:rect(rect.ax + 1, rect.ay + 1, rect.bx - 1, rect.by - 1, {
					color = fg, opacity = visibility * opacity, radius = math.max(0, state.radius - 1),
				})
			end
		...
		if title_bx - title_ax > self.font_size * 3 and options.top_bar_title ~= 'no' then
			...
			if main_title then
				...
				ass:rect(title_rect.ax, title_rect.ay, title_rect.bx, title_rect.by, {
					color = 'ffffff', opacity = visibility * config.opacity.title * 0.3, radius = state.radius,
				})
				ass:rect(title_rect.ax + 1, title_rect.ay + 1, title_rect.bx - 1, title_rect.by - 1, {
					color = bg, opacity = visibility * config.opacity.title, radius = math.max(0, state.radius - 1),
				})
			...
			if alt_title and options.top_bar_alt_title_place == 'below' then
				...
				ass:rect(ax + 1, title_ay + 1, bx - 1, by - 1, {
					color = bg, opacity = visibility * config.opacity.title, radius = math.max(0, state.radius - 1),
				})
				local align = left_aligned and rect_ideal_width == rect_width and 6 or 4
			...
			if self.current_chapter then
				...
				ass:rect(time_ax + 1, rect.ay + 1, time_bx - 1, rect.by - 1, {
					color = bg, opacity = visibility * config.opacity.title, radius = math.max(0, state.radius - 1),
				})
