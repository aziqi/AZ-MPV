...
Docs: https://github.com/hooke007/mpv_PlayKit/discussions/186

Minimalist design-driven multi-function UI script suite, compatible with the thumbfast thumbnail engine
]]
...
defaults = {
	...
	brightness_persistency = 'idle,audio',
...
local config_defaults = {
	color = {
		...
		background = serialize_rgba('101216').color,
...
config = {
	...
	cursor_leave_fadeout_elements = {'timeline', 'volume', 'brightness', 'top_bar', 'controls'},
	timeline_step = 1,
	timeline_step_flag = '',
}
...
function update_config()
	...
	for _, name in ipairs({'timeline', 'controls', 'volume', 'brightness', 'top_bar', 'speed'}) do
...
function update_display_dimensions()
	...
	-- Display resolution info is available from this point
...
mp.observe_property('brightness', 'number', create_state_setter('brightness'))
...
--bind_command('subtitles', create_select_tracklist_type_menu_opener({ -- Legacy secondary subtitle menu
...
-- Automatically show context menu when idle
...
local constructors = {
	...
	brightness = itable_index_of({'left', 'right'}, options.volume) and require('elements/Brightness'),
}
