...
local prop_actions = {
	{name = "copy", icon = "content_copy", label = "Copy property name and value"},
	{name = "show", icon = "visibility",   label = "OSD display full value"},
}
...
local function build_menu_data()
	...
	return {
		type = menu_type,
		title = "Property Check Menu",
		hint = tostring(#items) .. " items",
...
function handle_prop_menu_event(json_str)
	...
	if event.type == "activate" then
		...
		if action == "copy" then
			...
			mp.osd_message("Copied: " .. prop_key, 2)
