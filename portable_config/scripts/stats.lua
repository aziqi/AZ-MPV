...
Precondition for mpv.conf: --load-stats-overlay=no
Example key bindings (write in input.conf):

 <KEY>   script-binding display-stats            # Temporary stats display
 <KEY>   script-binding display-stats-toggle     # Permanent stats display
 <KEY>   script-binding display-page-1           # Temporary page 1
 ...
 <KEY>   script-binding display-page-1-toggle    # Permanent page 1
...
local o = {
    ...
    redraw_delay = 1,
    ass_formatting = true,
    persistent_overlay = true,       -- Prevents other OSD from overwriting (default true here)
...
if o.load == false then
    mp.msg.info("Script disabled by initialization")
    return
end
...
mp.observe_property("load-stats-overlay", "bool", function(_, value)
    if value == true then
        ...
        mp.msg.warn("Please start mpv with --load-stats-overlay=no to avoid script conflicts.")
    end
end)
...
local function text_style()
    ...
    if o.custom_header and o.custom_header ~= "" then
        ...
        if o.font_color ~= "" then style = style .. "\\1c&H" .. o.font_color .. "&\\1a&H" .. o.alpha .. "&" end
        if o.border_color ~= "" then style = style .. "\\3c&H" .. o.border_color .. "&\\3a&H" .. o.alpha .. "&" end
        if o.shadow_color ~= "" then style = style .. "\\4c&H" .. o.shadow_color .. "&\\4a&H" .. o.alpha .. "&" end
...
local function append_property(s, prop, attr, excluded, cached)
    ...
    local ret
    if cached then ret = get_property_cached(prop) else ret = mp.get_property_osd(prop) end
...
local function scroll_hint(search)
    local hint = format(" (Hint: Use %s/%s to scroll", o.key_scroll_up, o.key_scroll_down)
    if search then hint = hint .. " and " .. o.key_search .. " to search)" else hint = hint .. ")" end
    if not o.use_ass then return " " .. hint end
    return format(" {\\fs%s}%s{\\fs%s}", font_size * 0.66, hint, font_size)
end

local function make_frame_time_bar(time_us)
    local display_fps = mp.get_property_number("display-fps") or mp.get_property_number("container-fps") or 60.0
    if display_fps <= 0 then display_fps = 60.0 end
    local frame_budget_us = (1.0 / display_fps) * 1000000.0
    
    local ratio = time_us / frame_budget_us
    local bar_len = 10
    local filled_len = math.floor(ratio * bar_len + 0.5)
    filled_len = math.max(0, math.min(bar_len, filled_len))
    ...
    local color = "{\\1c&H80ee4a&}" -- green
    if ratio > 0.9 then
        color = "{\\1c&H7171f8&}" -- red
    elseif ratio > 0.7 then
        color = "{\\1c&H15ccfa&}" -- yellow
    end
    
    local bar = color .. string.rep("█", filled_len) .. "{\\1c&H695547&}" .. string.rep("░", empty_len)
    return bar .. text_style()
end
...
local function append_perfdata(header, s, dedicated_page)
    ...
    h[#h+1] = format("%s%s%s%s%s%s%s%s",
                     ...
                     bold("Frame Timing:"), o.prefix_sep, font_small,
                     "(last/avg/peak μs)", font_normal,
    ...
    for _,frame in ipairs(sorted_keys(vo_p)) do
        ...
        if dedicated_page then
            ...
            for _, pass in ipairs(data) do
                s[#s+1] = format(f, o.nl, o.indent, o.indent, font_mono, pp(pass["last"]),
                                 pp(pass["avg"]), pp(pass["peak"]), o.prefix_sep .. indent, 
                                 p(pass["last"], last_s[frame]), font, o.prefix_sep, o.prefix_sep, pass["desc"])
                s[#s] = s[#s] .. o.prefix_sep .. make_frame_time_bar(pass["last"])
            ...
            s[#s+1] = format(f, o.nl, o.indent, o.indent, font_mono, pp(last_s[frame]),
                             pp(avg_s[frame]), pp(peak_s[frame]), o.prefix_sep, bold("Total"), font, "", "", "")
            s[#s] = s[#s] .. o.prefix_sep .. make_frame_time_bar(last_s[frame])
        else
            s[#s+1] = format(f, o.nl, o.indent, o.indent, font_mono, pp(last_s[frame]), pp(avg_s[frame]), pp(peak_s[frame]),
                            "", "", font, o.prefix_sep, o.prefix_sep, frame:gsub("^%l", string.upper))
            s[#s] = s[#s] .. o.prefix_sep .. make_frame_time_bar(last_s[frame])
        end
    end
end

local cmd_prefixes = { osd_auto=1, no_osd=1, osd_bar=1, osd_msg=1, osd_msg_bar=1, raw=1, sync=1, async=1, expand_properties=1, repeatable=1, nonrepeatable=1, nonscalable=1, set=1, add=1, multiply=1, toggle=1, cycle=1, cycle_values=1, ["!reverse"]=1, change_list=1 }
local name_prefixes = { define=1, delete=1, enable=1, disable=1, dump=1, write=1, drop=1, revert=1, ab=1, hr=1, secondary=1, current=1 }

local function cmd_subject(cmd)
    cmd = cmd:gsub(";.*", ""):gsub("%-", "_")
    local TOKEN = '^%s*["\']?([%w_!]*)'
    local tok, sname, subw
    repeat tok, cmd = cmd:match(TOKEN .. '["\']?(.*)') until not cmd_prefixes[tok]
    ...
    repeat subw, tok = tok:match("([^_]*)_?(.*)") until tok == "" or not name_prefixes[subw]
...
local function keyname_cells(k)
    ...
    if klen > 1 and k:byte(klen) >= 0x80 then
        repeat klen = klen-1 until klen == 1 or k:byte(klen) >= 0xc0
    end
    return klen
end
...
local function get_kbinfo_lines()
    ...
    for _, bind in pairs(active) do
        ...
        if bind.subject ~= "ignore" then
            ...
            if bind.key:len() > kspaces:len() then kspaces = string.rep(" ", bind.key:len()) end
        end
    end
    local function align_right(key) return kspaces:sub(keyname_cells(key)) .. key end
    ...
    for _, bind in ipairs(ordered) do
        ...
        if bind.comment then bind.cmd = bind.cmd .. "  # " .. bind.comment end
...
local function append_display_sync(s)
    ...
    local vspeed = append_property(s, "video-speed-correction", {prefix="Display Sync Factor:"})
    if vspeed then
        append_property(s, "audio-speed-correction", {prefix="/", nl="", indent=" ", prefix_sep=" ", no_prefix_markup=true})
    else
        append_property(s, "audio-speed-correction", {prefix="Display Sync Factor:" .. o.prefix_sep .. " - / ", prefix_sep=""})
    end
    ...
    if not display_timer.oneshot and (o.plot_vsync_ratio or o.plot_vsync_jitter) and o.use_ass then
        local ratio_graph = ""
        local jitter_graph = ""
        if o.plot_vsync_ratio then ratio_graph = generate_graph(vsratio_buf, vsratio_buf.pos, vsratio_buf.len, vsratio_buf.max, nil, 0.8, 1) end
        if o.plot_vsync_jitter then jitter_graph = generate_graph(vsjitter_buf, vsjitter_buf.pos, vsjitter_buf.len, vsjitter_buf.max, nil, 0.8, 1) end
...
local function append_filters(s, prop, prefix)
    ...
    for _,f in ipairs(mp.get_property_native(prop, {})) do
        ...
        local p = {}
        for _,key in ipairs(sorted_keys(f.params)) do p[#p+1] = key .. "=" .. f.params[key] end
        p = #p > 0 and " [" .. table.concat(p, " ") .. "]" or ""
        length = length + n:len() + p:len()
        filters[#filters+1] = no_ASS(n) .. it(no_ASS(p))
    end
    if #filters > 0 then
        local ret = length < o.filter_params_max_length and table.concat(filters, ", ") or o.nl .. o.indent .. o.indent .. table.concat(filters, o.nl .. o.indent .. o.indent)
        s[#s+1] = o.nl .. o.indent .. bold(prefix) .. o.prefix_sep .. ret
    end
end

local function add_header(s) s[#s+1] = text_style() end
...
local function add_file(s, print_cache, print_tags)
    ...
    if mp.get_property_osd("filename") ~= mp.get_property_osd("media-title") then
        append_property(s, "media-title", {prefix="Title/Filename:"})
    end
    if print_tags then
    ...
    if ch_index and ch_index >= 0 then
        append_property(s, "chapter-list/" .. tostring(ch_index) .. "/title", {prefix="Chapter:", nl=(edition and editions > 1) and "" or o.nl})
...
local function crop_noop(w, h, r) return r["crop-x"] == 0 and r["crop-y"] == 0 and r["crop-w"] == w and r["crop-h"] == h end
local function crop_equal(r, ro) return r["crop-x"] == ro["crop-x"] and r["crop-y"] == ro["crop-y"] and r["crop-w"] == ro["crop-w"] and r["crop-h"] == ro["crop-h"] end
...
local function append_resolution(s, r, prefix, w_prop, h_prop, video_res)
    ...
    if append(s, r[w_prop], {prefix=prefix}) then
        ...
        if r["s"] then append(s, format("%.2f", r["s"]), {prefix="(", suffix="x)", nl="", indent=o.prefix_sep, prefix_sep="", no_prefix_markup=true}) end
...
local function pq_eotf(x)
    ...
    local PQ_M1, PQ_M2, PQ_C1, PQ_C2, PQ_C3 = 2610.0 / 16384, 2523.0 / 32, 3424.0 / 4096, 2413.0 / 128, 2392.0 / 128
    x = x ^ (1.0 / PQ_M2)
...
local function append_hdr(s, hdr, video_out)
    ...
    local function has(val, target) return val and math.abs(val - target) > 1e-4 end
    ...
    if has_dml or has_cll or has_fall then
        ...
        if has_cll then
            append(s, string.format("%.0f", hdr["max-cll"]), {prefix="MaxCLL:", suffix=" cd/m²", nl="", indent=indent})
            indent = o.prefix_sep .. o.prefix_sep
        end
        if has_fall then append(s, hdr["max-fall"], {prefix="MaxFALL:", suffix=" cd/m²", nl="", indent=indent}) end
    end
    indent = o.prefix_sep .. o.prefix_sep
...
local function append_img_params(s, r, ro)
    ...
    if ro and (r["w"] ~= ro["dw"] or r["h"] ~= ro["dh"]) then
        if ro["crop-w"] and (crop_noop(r["w"], r["h"], ro) or crop_equal(r, ro)) then ro["crop-w"] = nil end
        append_resolution(s, ro, "Output Res:", "dw", "dh")
    end
    local indent = o.prefix_sep .. o.prefix_sep
    r = ro or r
    append(s, r["hw-pixelformat"] or r["pixelformat"], {prefix="Pixel Format:"})
    append(s, r["colorlevels"], {prefix="Dynamic Range:", nl="", indent=indent})
    if r["chroma-location"] and r["chroma-location"] ~= "unknown" then append(s, r["chroma-location"], {prefix="Chroma Location:", nl="", indent=indent}) end
    append(s, r["colormatrix"], {prefix="Matrix Coeff:"})
    if r["prim-red-x"] then
        append(s, string.format("[%.3f %.3f, %.3f %.3f, %.3f %.3f, %.3f %.3f]", r["prim-red-x"] or 0, r["prim-red-y"] or 0, r["prim-green-x"] or 0, r["prim-green-y"] or 0, r["prim-blue-x"] or 0, r["prim-blue-y"] or 0, r["prim-white-x"] or 0, r["prim-white-y"] or 0), {prefix="Primaries:", nl="", indent=indent})
        append(s, r["primaries"], {prefix="in", nl="", indent=" ", prefix_sep=" ", no_prefix_markup=true})
    else
        append(s, r["primaries"], {prefix="Primaries:", nl="", indent=indent})
    end
    append(s, r["gamma"], {prefix="Transfer Char:", nl="", indent=indent})
end
...
local function append_fps(s, prop, eprop)
    ...
    local nl, indent = o.nl, o.indent
    if fps ~= "" and append(s, fps, {prefix=prefix, suffix=unit .. suffix}) then prefix, nl, indent = "", "", "" end
    if not single and efps ~= "" then append(s, efps, {prefix=prefix, suffix=unit .. esuffix, nl=nl, indent=indent}) end
end

local function add_video_out(s)
    local vo = mp.get_property_native("current-vo")
    if not vo then return end
    append(s, "", {prefix="Display Device:", nl=o.nl .. o.nl, indent=""})
    append(s, vo, {prefix_sep="", nl="", indent=""})
    append_property(s, "display-names", {prefix_sep="", prefix="(", suffix=")", no_prefix_markup=true, nl="", indent=" "}, nil, true)
    append(s, mp.get_property_native("current-gpu-context"), {prefix="GPU Context:", nl="", indent=o.prefix_sep .. o.prefix_sep})
    append_property(s, "avsync", {prefix="A/V Sync Offset:"})
    ...
    if mp.get_property_native("deinterlace-active") then append_property(s, "deinterlace", {prefix="Deinterlace:"}) end
    local scale = not mp.get_property_native("fullscreen") and get_property_cached("current-window-scale") or nil
    local od, rt = mp.get_property_native("osd-dimensions"), mp.get_property_native("video-target-params")
    local r = rt or {}
    r["s"] = scale
    r["crop-x"], r["crop-y"] = od["ml"], od["mt"]
    r["crop-w"], r["crop-h"] = od["w"] - od["ml"] - od["mr"], od["h"] - od["mt"] - od["mb"]
    if not rt then
        r["w"], r["h"] = r["crop-w"], r["crop-h"]
        append_resolution(s, r, "Resolution:", "w", "h", true)
        return
    end
...
local function add_video(s)
    local r, ro = mp.get_property_native("video-params"), mp.get_property_native("video-out-params")
    r = r or ro
    if not r then return end
    local track = mp.get_property_native("current-tracks/video")
    append(s, "", {prefix=(track and track.image) and "Image:" or "Video Track:", nl=o.nl .. o.nl, indent=""})
    if track and append(s, track["codec-desc"], {prefix_sep="", nl="", indent=""}) then
        ...
        if track["codec"] ~= track["decoder"] then append(s, track["decoder"], {prefix="[", nl="", indent=" ", prefix_sep="", no_prefix_markup=true, suffix="]"}) end
        append_property(s, "hwdec-current", {prefix="Hardware Dec:", nl="", indent=o.prefix_sep .. o.prefix_sep, no_prefix_markup=false, suffix=""}, {no=true, [""]=true}, true)
    end
    ...
    if o.show_frame_info then
        ...
        if frame_info and frame_info["picture-type"] then
            append(s, frame_info["picture-type"], has_prefix and {prefix="(", suffix=")", indent=" ", nl="", prefix_sep="", no_prefix_markup=true} or {prefix="Frame Type:"})
            has_prefix = true
        end
        if frame_info and frame_info["interlaced"] then append(s, "Interlaced", has_prefix and {indent=" ", nl="", prefix_sep=""} or {prefix="Frame Type:"}) end
        local timecodes = { ["gop-timecode"] = "GOP", ["smpte-timecode"] = "SMPTE", ["estimated-smpte-timecode"] = "Est. SMPTE" }
        for prop, name in pairs(timecodes) do
            if frame_info and frame_info[prop] then
                append(s, frame_info[prop], {prefix=name .. " Timecode:", indent=has_prefix and o.prefix_sep .. o.prefix_sep or "", nl=has_prefix and "" or nil})
                break
            end
        end
    end
    if mp.get_property_native("current-tracks/video/image") == false then append_fps(s, "container-fps", "estimated-vf-fps") end
    append_img_params(s, r, ro)
    append_hdr(s, ro)
    append_property(s, "video-bitrate", {prefix="Bitrate:"})
    append_filters(s, "vf", "Video Filters:")
end

local function add_audio(s)
    local r, ro = mp.get_property_native("audio-params"), mp.get_property_native("audio-out-params") or mp.get_property_native("audio-params")
    if not r and not ro then return end
    local merge = function(rr, rro, prop)
        local a, b = rr[prop] or rro[prop], rro[prop] or rr[prop]
        return (a == b or a == nil) and a or (a .. " ➜ " .. b)
    end
    append(s, "", {prefix="Audio Track:", nl=o.nl .. o.nl, indent=""})
    ...
    append_property(s, "current-ao", {prefix="Current AO:", nl="", indent=o.prefix_sep .. o.prefix_sep})
    ...
    if math.abs(mp.get_property_native("audio-delay")) > 1e-6 then append_property(s, "audio-delay", {prefix="Audio Delay:"}) end
    ...
    append_filters(s, "af", "Audio Filters:")
end

local function eval_ass_formatting()
    o.use_ass = o.ass_formatting and has_vo_window()
    if o.use_ass then
        o.nl, o.indent, o.prefix_sep, o.b1, o.b0, o.it1, o.it0 = o.ass_nl, o.ass_indent, o.ass_prefix_sep, o.ass_b1, o.ass_b0, o.ass_it1, o.ass_it0
    else
        o.nl, o.indent, o.prefix_sep, o.b1, o.b0, o.it1, o.it0 = o.no_ass_nl, o.no_ass_indent, o.no_ass_prefix_sep, o.no_ass_b1, o.no_ass_b0, o.no_ass_it1, o.no_ass_it0
    end
end
...
local function split(str, pat, plain)
    local init, r, i = 1, {}, 1
    repeat
        local f0, f1 = string.find(str, pat, init, plain)
        r[i], i = string.sub(str, init, f0 and f0 - 1), i+1
        init = f0 and f1 + 1
    until f0 == nil
    return r
end
...
local function keybinding_info(after_scroll, bindlist)
    local header, page = {}, pages[o.key_page_4]
    eval_ass_formatting()
    add_header(header)
    append(header, "", {prefix=bindlist and page.desc or page.desc .. ":" .. scroll_hint(true), nl="", indent=""})
    header = {table.concat(header)}
    if not kbinfo_lines or not after_scroll then kbinfo_lines = get_kbinfo_lines() end
    return finalize_page(header, kbinfo_lines, not bindlist)
end

local function float2rational(x)
    local max_den, m00, m01, m10, m11 = 100000, 1, 0, 0, 1
    local a, frac = math.floor(x), x - math.floor(x)
    while m10 * a + m11 <= max_den do
        ...
        x = 1 / frac
        a, frac = math.floor(x), x - math.floor(x)
    end
    return m00, m10
end
...
local function add_track(c, t, i)
    ...
    append(c, t["external-filename"], {prefix="External File:"})
    ...
    if append(c, t["codec-desc"], {prefix="Codec:"}) then
        ...
        if t["codec"] ~= t["decoder"] then append(c, t["decoder"], {prefix="[", nl="", indent=" ", prefix_sep="", no_prefix_markup=true, suffix="]"}) end
    end
    ...
    if not t["image"] and t["demux-fps"] then append_fps(c, "track-list/" .. i .. "/demux-fps", "") end
    ...
    if track_rg or album_rg then append(c, "", {prefix="ReplayGain:"}) end
    if track_rg then
    ...
    if t["dolby-vision-profile"] then
        append(c, "", {prefix="Dolby Vision:"})
        append(c, t["dolby-vision-profile"], {prefix="Profile:", nl="", indent=""})
        append(c, t["dolby-vision-level"], {prefix="Level:", nl="", indent=o.prefix_sep .. o.prefix_sep})
    end
end
...
local function track_info()
    ...
    eval_ass_formatting()
    add_header(h)
    append(h, "", {prefix=pages[o.key_page_5].desc .. ":" .. scroll_hint(), nl="", indent=""})
    h = {table.concat(h)}
    table.insert(c, o.nl .. o.nl)
    add_file(c, false, true)
    for i, track in ipairs(mp.get_property_native("track-list")) do
        if track['selected'] then add_track(c, track, i - 1) end
    end
...
local function perf_stats()
    ...
    eval_ass_formatting()
    add_header(header)
    append(header, "", {prefix=pages[o.key_page_0].desc .. ":" .. scroll_hint(), nl="", indent=""})
    append_general_perfdata(content)
    header = {table.concat(header)}
    return finalize_page(header, content, true)
end

local function opt_time(t) return type(t) == "number" and mp.format_time(t) or "?" end
...
local function cache_stats()
    ...
    local a, b = info["reader-pts"], info["cache-end"]
    append(stats, opt_time(a) .. " - " .. opt_time(b), {prefix = "Packet Queue:"})
    local r = (a ~= nil and b ~= nil) and b - a or nil
    local r_graph = ""
    if not display_timer.oneshot and o.use_ass and o.plot_cache then
        r_graph = o.prefix_sep .. generate_graph(cache_ahead_buf, cache_ahead_buf.pos, cache_ahead_buf.len, cache_ahead_buf.max, nil, 0.8, 1)
    end
    append(stats, opt_time(r), {prefix = "Read-ahead:", suffix = r_graph})
    local state = "Reading"
    if info["debug-seeking"] then state = "Seeking (to " .. mp.format_time(info["debug-seeking"]) .. ")"
    elseif info["eof"] then state = "End of File"
    elseif info["underrun"] then state = "Underflow"
    elseif info["idle"] then state = "Idle" end
    append(stats, state, {prefix = "State:"})
    local speed = info["raw-input-rate"] or 0
    local speed_graph = ""
    if not display_timer.oneshot and o.use_ass and o.plot_cache then
        speed_graph = o.prefix_sep .. generate_graph(cache_speed_buf, cache_speed_buf.pos, cache_speed_buf.len, cache_speed_buf.max, nil, 0.8, 1)
    end
    append(stats, utils.format_bytes_humanized(speed) .. "/s", {prefix="Speed:", suffix=speed_graph})
    append(stats, utils.format_bytes_humanized(info["total-bytes"]), {prefix = "Total RAM Size:"})
    append(stats, utils.format_bytes_humanized(info["fw-bytes"]), {prefix = "Est. RAM Size:"})
    append(stats, info["file-cache-bytes"] and utils.format_bytes_humanized(info["file-cache-bytes"]) or "(disabled)", {prefix = "Local File Cache:"})
    append(stats, info["debug-low-level-seeks"], {prefix = "Media Seek Count:"})
    append(stats, info["debug-byte-level-seeks"], {prefix = "Stream Seek Count:"})
    append(stats, "", {prefix="Ranges:", nl=o.nl .. o.nl, indent=""})
    append(stats, info["bof-cached"] and "yes" or "no", {prefix = "Start Buffered:"})
    append(stats, info["eof-cached"] and "yes" or "no", {prefix = "End Buffered:"})
...
local function record_cache_stats()
    ...
    if info["reader-pts"] and info["cache-end"] then graph_add_value(cache_ahead_buf, info["cache-end"] - info["reader-pts"]) end
...
pages = {
    ...
    [o.key_page_0] = { idx = 1, f = perf_stats, desc = "Internal Performance", scroll = true },
}
...
local function record_data(skip)
    ...
    return function()
        ...
        if o.plot_vsync_jitter then
            ...
            if r then graph_add_value(vsjitter_buf, r) end
        end
        if o.plot_vsync_ratio then
            local r = mp.get_property_number("vsync-ratio")
            if r then graph_add_value(vsratio_buf, r) end
        end
    end
end
...
update_scale = function ()
    local scale_with_video = (o.vidscale == "auto") and mp.get_property_native("osd-scale-by-window") or (o.vidscale == "yes")
    local scale = 288 / 720
    local osd_height = mp.get_property_native("osd-height")
    if not scale_with_video and osd_height > 0 then scale = 288 / osd_height end
    font_size, border_size, shadow_x_offset, shadow_y_offset, plot_bg_border_width = o.font_size * scale, o.border_size * scale, o.shadow_x_offset * scale, o.shadow_y_offset * scale, o.plot_bg_border_width * scale
    if display_timer:is_enabled() then print_page(curr_page) end
end

local function clear_screen() if o.persistent_overlay then mp.set_osd_ass(0, 0, "") else mp.osd_message("", 0) end end
...
local function reset_scroll_offsets() for _, page in pairs(pages) do page.offset = nil end end
...
local function filter_bindings()
    input.get({
        ...
        opened = function () searched_text = "" end,
...
local function bind_search() mp.add_forced_key_binding(o.key_search, "__forced_"..o.key_search, filter_bindings) end
local function unbind_search() mp.remove_key_binding("__forced_"..o.key_search) end
local function bind_exit() if not display_timer.oneshot then mp.add_forced_key_binding(o.key_exit, "__forced_" .. o.key_exit, function () process_key_binding(false) end) end end
local function unbind_exit() mp.remove_key_binding("__forced_" .. o.key_exit) end
...
local function add_page_bindings()
    ...
    for k, _ in pairs(pages) do mp.add_forced_key_binding(k, "__forced_"..k, a(k), {repeatable=true}) end
    update_scroll_bindings(curr_page)
    bind_exit()
end

local function remove_page_bindings()
    for k, _ in pairs(pages) do mp.remove_key_binding("__forced_"..k) end
    unbind_scroll()
    unbind_search()
    unbind_exit()
end
...
process_key_binding = function(oneshot)
    ...
    if display_timer:is_enabled() then
        if display_timer.oneshot and oneshot then
            display_timer:kill() ; print_page(curr_page) ; display_timer:resume()
        elseif not display_timer.oneshot and not oneshot then
            display_timer:kill() ; cache_recorder_timer:stop()
            if tm_viz_prev ~= nil then mp.set_property_native("tone-mapping-visualize", tm_viz_prev) ; tm_viz_prev = nil end
            clear_screen() ; remove_page_bindings()
            if recorder then mp.unobserve_property(recorder) ; recorder = nil end
        end
    else
        ...
        if not oneshot then
            cache_ahead_buf, cache_speed_buf = {0, pos = 1, len = 50, max = 0}, {0, pos = 1, len = 50, max = 0}
            cache_recorder_timer:resume()
        end
        display_timer:kill()
        display_timer.oneshot = oneshot
        display_timer.timeout = oneshot and o.duration or o.redraw_delay
        add_page_bindings() ; print_page(curr_page) ; display_timer:resume()
    end
end

display_timer = mp.add_periodic_timer(o.duration, function()
    if display_timer.oneshot then display_timer:kill() ; clear_screen() ; remove_page_bindings() ; if searched_text then input.terminate() end
    else print_page(curr_page) end
end)
display_timer:kill()

mp.add_key_binding(nil, "display-stats", function() process_key_binding(true) end, {repeatable=true})
mp.add_key_binding(nil, "display-stats-toggle", function() process_key_binding(false) end, {repeatable=false})

for k, page in pairs(pages) do
    mp.add_key_binding(nil, "display-page-" .. page.idx, function() curr_page = k ; process_key_binding(true) end, {repeatable=true})
    mp.add_key_binding(nil, "display-page-" .. page.idx .. "-toggle", function() curr_page = k ; process_key_binding(false) end, {repeatable=false})
end

mp.register_event("video-reconfig", function() if display_timer:is_enabled() and not display_timer.oneshot then print_page(curr_page) end end)
...
if o.bindlist ~= "no" then
    ...
    mp.add_timeout(0, function()
        if o.bindlist:sub(1, 1) == "-" then o.no_ass_b0, o.no_ass_b1 = "", "" end
        o.ass_formatting, o.no_ass_indent = false, " "
        mp.osd_message(keybinding_info(false, true))
        mp.add_timeout(0, function() mp.command("flush-status-line no") ; mp.command("quit") end)
    end)
end

mp.observe_property("osd-height", "native", update_scale)
mp.observe_property("osd-scale-by-window", "native", update_scale)
mp.observe_property('current-window-scale', 'native', function(n, v) property_cache[n] = v end)
mp.observe_property('display-names', 'string', function(n, v) property_cache[n] = v end)
mp.observe_property('hwdec-current', 'string', function(n, v) property_cache[n] = v end)