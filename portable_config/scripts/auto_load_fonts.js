 ...
 * 
 * Automatically configure fontconfig to load font files from the 'fonts' folder in the current media file's directory
 *
 * Requires sub-font-provider=fontconfig
 * 
 * Some NTFS partitions on Windows may fail to load font files with non-English characters in their names. Compatibility mode can be used to work around this.
 * Compatibility mode info: https://github.com/hooke007/mpv_PlayKit/discussions/189
 */
...
function update_options() {
    ...
    // Clean up old directory if the compatible directory has changed
...
function write_fonts_conf(fonts_dir, require_exist) {
    ...
    // Perform some checks to avoid unnecessary repeated writes.
...
(function () {
    mp.add_hook('on_load', 50, function () {
        ...
        // If there is no font directory but one was set before, write an empty config file.
        if (!fonts_dir_exist) {
